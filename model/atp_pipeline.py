...
class atp_pipeline(keras.models.Model):
    ...
    # # Define loop condition (outside of this function)
    # def condition(i, ts_start, inputs_for_processing, labels_len):
    #     return i < labels_len

    # # Initialize loop variables (e.g., i, ts_start, inputs_for_processing)
    # i = tf.constant(0)
    # ts_start = tf.constant(0)
    # inputs_for_processing = tf.TensorArray(dtype=tf.float32, size=0, dynamic_size=True, clear_after_read=False)
    # labels_len = tf.shape(y)[0] // 180
    # # Execute the loop
    ...
    def call(self, inputs):
        ...
        if self.multiply == 1:
            ...
            # # the end sequence will be (y_11,.., y_1n_C, y21, ..y_2n_C, y1*, y2*, ..yk*, y1**, y2**, ..yk**)
            # print("x shape before inputs_for_processing:", x.shape)
            # print("y shape before inputs_for_processing:", y.shape)
        ...
        if self._subsample:
            inputs_for_processing.append(5)
        else:
        ...
        ######## make mask #######
        if self._subsample:
            mask = self._feature_wrapper.masker(n_C1, 5)
        else:
