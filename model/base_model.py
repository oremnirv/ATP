...
class ATP(tf.keras.Model):
    ...
    def call(self, input, training=True):
        x_emb, y, n_C, n_T, _, mask = input
        batch_s = tf.shape(y)[0]

        value_xy_label = tf.zeros((batch_s,  n_C+n_T,  1))
        value_xy = tf.concat([y,  value_xy_label,  x_emb], axis=-1)
        query_xy_label = tf.concat([tf.zeros((batch_s,  n_C,  1)), tf.ones((batch_s,  n_T,  1))],  axis=1)
        query_y = tf.concat([y[:,  :n_C],  tf.zeros((batch_s,  n_T,  1))],  axis=1)
        query_xy = tf.concat([query_y, query_xy_label,  x_emb], axis=-1)

        xy = self.mha_xy_a(query_xy, value_xy, value_xy, mask,training=training)
