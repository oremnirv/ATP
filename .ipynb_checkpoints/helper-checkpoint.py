...
def make_features(t, y, context_points, batch_s=32):
    x = embed_position(t, d=28, TΔmin=0.1, Tmax=2)
...
def embed_position(t, d=28, TΔmin=0.1, Tmax=2):  # return.shape=(T,B,d)
...
def derivative_extr(ŷ, x̂, c, embed=False):
    ...
    for i in range(m):
        for j in range(c):
            ...
            x_n[i, j, :] = x_temp[ix]
            if embed:
                diff_x[i, j, :] = embed_position(np.array([current_x])[:, np.newaxis]) -  embed_position(np.array([x_temp[ix]])[:, np.newaxis])
                x_n[i, j, :] = embed_position(np.array([x_temp[ix]])[:, np.newaxis])
        ...
        for j in range(c, ŷ.shape[1]):
            ...
            if embed:
                diff_x[i, j, :] = embed_position(np.array([x_temp[-1]])[:, np.newaxis]) -  embed_position(np.array([x_temp[ix]])[:, np.newaxis])
                x_n[i, j, :] = embed_position(np.array([x_temp[ix]])[:, np.newaxis])
            
            
            y_n[i, j] = y_temp[ix]

    return diff_y, diff_x, d, x_n, y_n