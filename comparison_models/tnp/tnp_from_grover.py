...
class FFN(tf.keras.layers.Layer):
    def __init__(self, d_model,dim_feedforward, dropout_rate=0.1):
        ...
        self.dense_b = tf.keras.layers.Dense(dim_feedforward)
...
class MHA_XY(tf.keras.layers.Layer):
    def __init__(self,
                 num_heads,
                  ...
                  dropout_rate=0.1):
        super().__init__()
        self.mha = dot_prod.MultiHeadAttention(num_heads, d_model, d_model)
        #d_model must be a multiple of num_heads
        self.ffn = FFN(d_model,dim_feedforward, dropout_rate)
...
class embed_layers(tf.keras.layers.Layer):
    def __init__(self,d_model,num_layers_embed=4):
        super().__init__()
        self.num_layers = num_layers_embed
        self.embed = [tf.keras.layers.Dense(d_model,activation="relu") for _ in range(self.num_layers-1)]
        self.embed.append(tf.keras.layers.Dense(d_model))
...
class TNP_Decoder(tf.keras.models.Model):
    def __init__(self,d_model=64,num_layers=6,dim_feedforward=128,
                 num_heads=4,dropout_rate=0.0,target_y_dim=1,bound_std=False):
        super().__init__()

        self.num_layers = num_layers

        self.mha_xy = [MHA_XY(num_heads,
                              d_model,
                              dim_feedforward,dropout_rate) for _ in range(num_layers)]

        self.embed = embed_layers(d_model,num_layers_embed=4)

        self.dense = tf.keras.layers.Dense(dim_feedforward,activation="relu")
